...
Let us now think about how we can use the smoothness of $H^{(k,Bloch)}$ to predict energies and wave-functions at finite $k$ from $H^{(k=0,Bloch)}$ and its derivatives. We start by expanding the Bloch Hamiltonian $$H^{(k,Bloch)}\approx H^{(k=0,Bloch)}+k H^{'(k=0,Bloch)}+(k^2/2)H^{''(k=0,Bloch)}$$. Using standard perturbation theory we can conclude that:
