...
def plot_wf(syst, wf1, wf2, lstyle="-", lcolor="b"):
    ...
    indx = np.argsort(xs)
    wf_sq = (
        np.linalg.norm(wf1.reshape(-1, 2), axis=1) ** 2
        + np.linalg.norm(wf2.reshape(-1, 2), axis=1) ** 2
    )
    plot = holoviews.Path((xs[indx], wf_sq[indx]), kdims=[dims.x, dims.amplitude])
    return plot.opts(
        style={"linestyle": lstyle, "color": lcolor},
...
def plot_pwave(L, t, delta, mu):
    ...
    return plot_wf(syst, evec[:, L], evec[:, L - 1]) * plot_wf(
        syst, evec[:, L + 1], evec[:, L - 2], lstyle="--", lcolor="r"
    )
...
(
    ...
    * holoviews.HoloMap({mu: holoviews.VLine(mu) for mu in mus}, kdims=[dims.mu_t])
    + holoviews.HoloMap(
        {mu: plot_pwave(25, 1.0, 1.0, mu) for mu in mus}, kdims=[dims.mu_t]
    )
)
```
...
plots = {
    (mu, Dirac_cone): bandstructure(mu, Dirac_cone=Dirac_cone)
    for mu in mus
...
mus = np.arange(-3, 3, 0.25)
holoviews.HoloMap(
    {mu: bandstructure(mu, show_pf=True) for mu in mus}, kdims=[dims.mu_t]
)
```
...
holoviews.HoloMap(
    {p.mu: spectrum(syst, p, **kwargs) for p.mu in mus}, kdims=[dims.mu_t]
)
```
