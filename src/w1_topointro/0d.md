...
def plot_hamiltonian_spectrum(alphas, spectrum, E_range=(-4, 4)):
    ...
    """
    E_min, E_max = E_range
    plot = holoviews.Path((alphas, spectrum), kdims=[dims.alpha, dims.E])[
        :, E_min:E_max
    ] * holoviews.HLine(0)
...
def plot_Q(alphas, Q, Q_range, Q_dim=dims.Q):
    ...
    Q_min, Q_max = Q_range
    Q_mid = (Q_max + Q_min) / 2
    plot = holoviews.Area((alphas, Q), kdims=[dims.alpha], vdims=[Q_dim])[
        :, Q_min:Q_max
    ].opts(style={"alpha": 0.4})
    plot *= holoviews.Curve((alphas, Q), kdims=[dims.alpha], vdims=[Q_dim])[
        :, Q_min:Q_max
    ]
    return plot.opts(
        plot={"xticks": [0, 0.5, 1], "yticks": [Q_min, Q_mid, Q_max], "aspect": 4}
    )
```
...
(
    ...
    + plot_Q(alphas, pfaffian, [-1.5, 1.5], dims.Q_BdG)
).cols(1)
```
