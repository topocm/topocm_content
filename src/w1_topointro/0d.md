...
def plot_hamiltonian_spectrum(alphas, spectrum, E_range=(-4, 4)):
    ...
    return plot.opts(holoviews.opts.Path(**{'xticks':[0, 0.5, 1], 'yticks':[E_min, 0, E_max]}))
...
def plot_Q(alphas, Q, Q_range, Q_dim=dims.Q):
    ...
    Q_min, Q_max = Q_range
    Q_mid = (Q_max + Q_min) / 2
    plot = holoviews.Area((alphas, Q), kdims=[dims.alpha], vdims=[Q_dim])[:, Q_min:Q_max].opts(style={'alpha': 0.4})
    plot *= holoviews.Curve((alphas, Q), kdims=[dims.alpha], vdims=[Q_dim])[:, Q_min:Q_max]
    return plot.opts(plot={'xticks':[0, 0.5, 1], 'yticks':[Q_min, Q_mid, Q_max], 'aspect':4})
```
