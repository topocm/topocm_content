...
When you have a pair of $c$ and $c^\dagger$ operators, you can write them in the following way:
...
All the states which are not at the ends of the chain have an energy of $\pm |t|$, independent of the length of the chain. Hence, we have a one-dimensional system with a gapped bulk and zero energy states at the edges.
...
def plot_two_lowest_eigenstates(syst, **params):
    ...
    return line_plot(
        np.arange(traces.shape[1]),
        traces.T,
        labels=["$\\psi_{0,1}$", "$\\psi_{2,3}$"],
        x_label=dims["x"],
        y_label=dims["amplitude"],
        show_legend=True,
        color=["#1f77b4", "#d62728"],
...
mus_display = np.arange(0, 4, 0.2)
mus_eval = mus_display + EPSILON
...
base_spectrum = spectrum(
    finite_chain,
    dict(t=1, delta=1, mu=mus_eval),
    **spectrum_kwargs,
)
for trace in base_spectrum.data:
    trace.x = mus_display


def frame(mu_display):
    mu_eval = mu_display + EPSILON
    wavefunctions = plot_two_lowest_eigenstates(
        finite_chain, t=1.0, delta=1.0, mu=mu_eval
    )
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=["Spectrum", "Lowest eigenstates"],
    ...
    fig.add_trace(
        dict(
            x=[mu_display, mu_display],
            y=[spectrum_kwargs["ylims"][0], spectrum_kwargs["ylims"][1]],
            mode="lines",
    ...
    fig.update_xaxes(
        title=dims["mu_t"], tickvals=spectrum_kwargs["xticks"], row=1, col=1
    )
    fig.update_yaxes(
        title=dims["E_t"],
        range=spectrum_kwargs["ylims"],
        tickvals=spectrum_kwargs["yticks"],
        row=1,
        col=1,
    )
    for trace in wavefunctions.data:
        fig.add_trace(trace, row=1, col=2)
    fig.update_xaxes(title=dims["x"], row=1, col=2)
    fig.update_yaxes(title=dims["amplitude"], row=1, col=2)
    fig.update_layout(
        title="Edge modes vs chemical potential",
        legend=dict(
            orientation="h",
            x=0.65,
            y=0.92,
...
slider_plot({mu: frame(mu) for mu in mus_display}, label="μ/t")
```
...
As we increase $\mu$ (try it), the wave function of the Majoranas becomes less localized near the edges of the wire, but the coupling between the two ends only appears later.
...
def band_structure(params, Dirac_cone="Hide", show_pf=False):
    ...
    if show_pf:
        ...
        title = rf"$\mu = {mu_str} t \;|\; \mathrm{{Pf}}[iH(0)]\,{signs[0]}\,0 \;|\; \mathrm{{Pf}}[iH(\pi)]\,{signs[1]}\,0$"
        add_reference_lines(plot, x=0, line_color="#666")
        add_reference_lines(plot, x=-np.pi, line_color="#666")
    else:
        title = rf"$\mu = {mu_str} t \;|\; \textrm{{{phase}}}$"
    ...
    plot.update_layout(title=dict(text=title), showlegend=False)
    return plot
...
plots = {
    params["mu"]: band_structure(params, Dirac_cone="Show", show_pf=False)
    for params["mu"] in mus
}
slider_plot(plots, label="μ/t")
```
...
slider_plot(
    {
        params["mu"]: band_structure(params, Dirac_cone="Hide", show_pf=True)
        for params["mu"] in mus
    },
    label="μ/t",
)
