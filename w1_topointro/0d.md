...
opts.defaults(
    opts.Layout(sublabel_format="", aspect_weight=1, fig_size=(100), vspace=0.4)
)
...
def plot_hamiltonian_spectrum(alphas, spectrum, E_range=(-4, 4)):
    ...
    plot = holoviews.Path(
        (alphas, spectrum), kdims=[dims["alpha"], energy]
    ) * holoviews.HLine(0)
    return plot.options(xticks=[0, 0.5, 1], yticks=[E_min, 0, E_max])
...
def plot_Q(alphas, Q, Q_range, Q_dim=dims["Q"]):
    ...
    plot = holoviews.Area((alphas, Q), kdims=[dims["alpha"]], vdims=[Q_dim]).options(
        alpha=0.4
    )
    plot *= holoviews.Curve((alphas, Q), kdims=[dims["alpha"]], vdims=[Q_dim])
    return plot.options(
        xticks=[0, 0.5, 1], yticks=[int(Q_min), int(Q_mid), int(Q_max)], aspect=4
    )
```
...
opts.defaults(opts.Layout(aspect_weight=1, fig_inches=(8, 6), fig_size=100))
np.random.seed(13)
...
:explanation: We cannot be sure about the conservation law, since the blocks may have different remaining symmetries. And we just saw that sublattice symmetry makes every system trivial, while spinful time-reversal makes the numbers of levels even.
:correct: 0
...
:explanation: If $\Delta=0$ the numbers of filled electron and hole states is conserved, and so the invariant once again becomes just the number of filled states.
:correct: 2
