...
from course.init_course import init_notebook, pprint_matrix
...
def stack(figures, cols=1):
    return combine_plots(list(figures), cols=cols)
...
def plot_hamiltonian_spectrum(alphas, spectrum, E_range=(-4, 4)):
    ...
    --------
    plot : plotly figure
        Plot of alphas vs. spectrum with a zero-energy reference.
    """
    E_min, E_max = E_range
    return line_plot(
        alphas,
...
def plot_Q(alphas, Q, Q_range, Q_dim=dims["Q"]):
    ...
    return area_plot(
        ...
        y_label=Q_dim,
        x_ticks=[0, 0.5, 1],
        y_ticks=[int(Q_min), int(Q_mid), int(Q_max)],
        y_range=Q_range,
        fill_opacity=0.25,
    )
```
...
In the following, we will see that one is often interested in a more specific criterion: for instance, that some symmetry is preserved throughout the continuous path that connects two Hamiltonians.
...
We see that one level does cross zero (even twice), but it seems obvious that we can just push it down a little bit and we find a continuous path between the two Hamiltonians. So we need to come up with an easier way to figure out if Hamiltonians can be transformed into each other or not.
...
Since this number cannot change under continuous transformations inside the set of gapped Hamiltonians, we call it a *topological invariant* $Q$.
...
As you can see, unitary symmetries play a rather boring role. As usual, they allow us to reduce the dimension of the problem at hand, but nothing more. There are, however, other symmetries which can have a rich influence on topology. An important example is time-reversal symmetry, as we will see next.

## Time-reversal symmetry

In our previous examples, you might have wondered whether there was anything special about choosing real matrices. Indeed there was something special. A real Hamiltonian is a manifestation of time-reversal symmetry. Time-reversal symmetry is represented by an anti-unitary operator, and as such it can always be written as the product $\mathcal{T}=U\mathcal{K}$ of a unitary matrix and complex conjugation. In the case above, we had simply $\mathcal{T}\equiv\mathcal{K}$. Our real Hamiltonians clearly obeyed time-reversal symmetry since $H=H^*$.
...
As you can see, there aren't many differences from the previous case. The different energy levels move, and the topological invariant changes when one of them crosses zero.
...
The following matrix is an example of such a Hamiltonian:
...
While the spectrum looks quite similar to the previous ones, whenever a line crosses zero energy, our topological invariant makes a jump of two, rather than one. In this case, time-reversal symmetry constrains the topological invariant to only take even values. This is an example of how topological properties can be influenced by discrete symmetries.
...
Let's now take a system where we can split all the degrees of freedom into two groups (say group $A$ and group $B$) such that the Hamiltonian only has nonzero matrix elements between the two groups, and not inside each group. This situation arises naturally when the lattice has two sublattices, as in the hexagonal carbon lattice of graphene. So let's imagine our quantum dot is now a graphene dot:
...
$(\psi_A, -\psi_B)^T$ is an eigenvector with energy $-\varepsilon$. This symmetry enforces a symmetric spectrum.

What does this mean for the topological classification?
Clearly, the number of states with negative energy is the same as the number of states with positive energy, so we don't ever expect a single level to cross zero energy.
...
```{multiple-choice} OK, let's see what we have so far. Which symmetry certainly does not restrict the values that the topological invariant can take?
:explanation: We cannot be sure about the conservation law, since the blocks may have different remaining symmetries. And we just saw that sublattice symmetry makes every system trivial, while spinful time-reversal symmetry makes the numbers of levels even.
:correct: 0
...
There is another symmetry that has a strong influence on topology: particle-hole symmetry. It shows up in superconducting systems. As an example, imagine bringing our quantum dot into contact with a superconductor, like this:

![](figures/SCdot.svg)

A superconductor will create and annihilate pairs of electrons by breaking apart and forming [Cooper pairs](https://en.wikipedia.org/wiki/Cooper_pair).
...
The $H$-term is the dynamics of the electrons in the dot, while $\Delta$ describes the pair creation and annihilation. The matrix $\Delta$ is antisymmetric because the fermion operators anticommute. Now $\mathcal{H}$ does not conserve the number of electrons, but it still conserves the parity of the number of electrons, that is, whether the number of electrons is even or odd. We can now group all the creation and annihilation operators in a vector, $C = (c_1, \dots, c_n, c^\dagger_1, \dots, c^\dagger_n)^T$. Then we write $\mathcal{H}$ in the form 'row multiplies matrix multiplies column':
...
The Bogoliubov-de Gennes Hamiltonian acts on wave functions whose first half is composed of annihilation operators of electrons, and the second half of creation operators of the same electrons.
We can think of them as annihilation operators of an extra set of holes, so that we double the number of degrees of freedom in the system.
...
Let's think a bit more about these crossings. At first, they might look a bit mysterious. In general, a crossing between energy levels happens in the presence of a conserved quantity, and our random Bogoliubov-de Gennes Hamiltonian does not seem to have an obvious one. However, let's recall what we said earlier: While the mean-field Hamiltonian of a superconductor does not conserve the number of particles, it conserves the parity of this number. In other words, forming and breaking Cooper pairs does not affect whether the superconducting quantum dot contains an even or odd number of electrons. In short, fermion parity is a conserved quantity (provided that isolated electrons do not enter or leave the dot, a possibility which we will disregard).

The above observation reveals that the existence of crossings is due to fermion parity conservation. Fermion parity, however, is a many-body quantity, which cannot be directly described in terms of the single particle picture of the Bogoliubov-de Gennes Hamiltonian.
...
Hence, unlike in the case of sublattice symmetry, a pair of $\pm E$ energy levels does not correspond to two distinct quantum states, but to a *single* quantum state.
This quantum state is a coherent superposition of electrons and holes: a *Bogoliubov quasiparticle* with excitation energy $E$, created by an operator $a^\dagger = u c^\dagger + v c$.
...
def find_pfaffian(alphas, H0, H1):
    """Function calculates the Pfaffian for a Hamiltonian.
...
```{multiple-choice} What happens to the topological invariant if we take the superconducting Hamiltonian and set $\Delta=0$?
:explanation: If $\Delta=0$, the numbers of filled electron and hole states are conserved, and so the invariant once again becomes just the number of filled states.
:correct: 2
