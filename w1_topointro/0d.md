...
opts.defaults(opts.Layout(sublabel_format='', aspect_weight=1, fig_size=(100), vspace=0.4))
...
def plot_Q(alphas, Q, Q_range, Q_dim=dims["Q"]):
    ...
    plot = holoviews.Area((alphas, Q), kdims=[dims["alpha"]], vdims=[Q_dim]).options(alpha=0.4)
    plot *= holoviews.Curve((alphas, Q), kdims=[dims["alpha"]], vdims=[Q_dim])
    return plot.options(xticks=[0, 0.5, 1], yticks=[int(Q_min), int(Q_mid), int(Q_max)], aspect=4)
```
