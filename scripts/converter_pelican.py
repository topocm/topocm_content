...
def notebook_title(nb: nbformat.NotebookNode):
    first_text_cell = next(cell for cell in nb.cells if cell.cell_type == "markdown")
    try:
...
for ipynb in ipynbs:
    ...
    nb.cells = list(dropwhile((lambda cell: cell.cell_type == "code"), nb.cells))
    ...
    with open(new_fname + "-meta", "w", encoding="utf-8") as f:
        slug = ipynb.replace("generated/with_output/", "").replace(".ipynb", "")
