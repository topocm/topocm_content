...
def split_into_units(nb_name):
    ...
    def split_cells():
        cells = dropwhile((lambda cell: cell.cell_type != "markdown"), nb.cells)
        for cell in cells:
            if cell.cell_type != "markdown":
                yield cell
            else:
                split_sources = re.split("(^# .*$)", cell.source, flags=re.MULTILINE)
                for src in split_sources:
                    yield nbformat.NotebookNode(
                        source=src, cell_type="markdown", metadata={},
                    )
    ...
    for cell in split_cells():
        if cell.cell_type == "markdown" and cell.source.startswith("# "):
            ...
            units.append(current.new_notebook(metadata={"name": nb_name}))
        else:
    ...
    for cell in cells:
        ...
        for output in cell.outputs:
            ...
            if data and "application/vnd.edx.olxml+xml" in data:
                xml_components.append(data["application/vnd.edx.olxml+xml"])
...
def converter(mooc_folder, content_folder=None):
    ...
    for chapter_number, chapter in enumerate(chapters):
        ...
        for section_number, section in enumerate(chapter["sections"]):
            ...
            units = split_into_units(content_folder / (section["location"] + ".ipynb"))
            ...
            for i, unit in enumerate(units):
                ...
                vertical = SubElement(
                    ...
                    "vertical",
                    attrib=dict(
                        url_name=vertical_url, display_name=unit.metadata.name,
                    ),
                )
    ...
    course_xml_path.write_text(ElementTree.tostring(xml_course, encoding="unicode"))
...
def expand_syllabus(toc, template, out):
    ...
    Path(out).write_text(
        jinja2.Template(Path(template).read_text()).render(
