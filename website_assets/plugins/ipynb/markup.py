...
from .ipynb import fix_css, get_html_from_filepath
    ...
    def read(self, filepath):
        ...
        if "summary" not in [key.lower() for key in self.settings.keys()]:
            ...
            if isinstance(
                ...
                content = "<body>{}</body>".format(content.decode("utf-8"))
            else:
            ...
            content = parser.body
            if (
                "IPYNB_USE_META_SUMMARY" in self.settings.keys()
                and self.settings["IPYNB_USE_META_SUMMARY"] == False
            ) or "IPYNB_USE_META_SUMMARY" not in self.settings.keys():
